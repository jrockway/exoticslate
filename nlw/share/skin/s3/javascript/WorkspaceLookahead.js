...
jQuery.extend(WorkspaceLookahead, LookaheadWidget);
...
WorkspaceLookahead.prototype._findSuggestions = function () {
    ...
    if (this.editField.value.length == 0) {
        ...
        if (this.previousText != this.editField.value) {
            ...
            jQuery.getJSON(uri, function (text) {
                self.populateSuggestion(text);
            });
        }
    }
}
 ...
 */
WorkspaceLookahead.prototype.populateSuggestion = function(text) {
    this._parseSuggestionList(text);
    if (!this.hasFocus) {
        return;
    }
...
WorkspaceLookahead.prototype._keyHandler = function (event) {
    var radioName = this.editFieldId + '-rb';
    jQuery('input[name='+radioName+'][value=other]').attr('checked', true)
