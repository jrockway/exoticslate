...
LookaheadWidget = function(dialog_window, api, edit_field_id, window_class, suggestion_block_class, suggestion_class, variable_name) {
    ...
    this.editField = jQuery('#' + edit_field_id).get(0);
...
LookaheadWidget.prototype._hookInterface = function () {
    var self = this;
    if (jQuery('#'+this.editFieldId).get(0)) {
        jQuery('#'+this.editFieldId).keyup(function () {
            self._findSuggestions();
        });
        jQuery('#'+this.editFieldId).keydown(function() {
            self._keyHandler();
        });
        jQuery('#'+this.editFieldId).blur(function() {
            self._loseFocus();
        });
        jQuery('#'+this.editFieldId).focus(function() {
            self._gainFocus();
        });
    }
}
...
LookaheadWidget.isRelative = function(node) {
...
LookaheadWidget.getDocumentX = function(e, is_relative) {
    var x = 0;
    while (e) {
        x+= e.offsetLeft;
        e = e.offsetParent;
...
LookaheadWidget.getDocumentY = function(e, is_relative) {
    var y = 0;
    while (e) {
        y += e.offsetTop;
        e = e.offsetParent;
        if (e && is_relative && this.isRelative(e))
            e = null;
    }
    return y;
}
...
LookaheadWidget.prototype._showSuggestionBlock = function () {
    ...
    this.suggestionWindow.style.left = this.getDocumentX(this.editField,true) + "px";
    this.suggestionWindow.style.top =
        this.getDocumentY(this.editField,true) +
