...
proto.form_values = function(widget, form) {
    ...
    for (var i = 0; i < fields.length; i++) {
        ...
        if (this.currentWidget.title_and_id && this.currentWidget.title_and_id[field] && this.currentWidget.title_and_id[field].id)
            ...
            value = ST.getRadioValue(field);
        else
            value = form[field].value.
                replace(/^\s*/, '').
                replace(/\s*$/, '');
        if (form['st-widget-' + field + '-rb']) {
            var whichValue = ST.getRadioValue('st-widget-' + field + '-rb');
...
proto.getWidgetInput = function(widget_element, selection, new_widget) {
    ...
    var callback = function(element) {
        ...
        if (form.size) {
            DOM.Events.addListener(form.width, "click", function () {
                form.size[4].checked = true;
                disable(form.height);
                enable(form.width);
            });
            DOM.Events.addListener(form.height, "click", function () {
                form.size[4].checked = true;
                disable(form.width);
                enable(form.height);
            });
...
Widget.Lightbox.Socialtext.prototype.applyHandlers = function(){
    ...
    if (Widget.Lightbox.is_ie) {
        var apply_style = function() {
            if (self && self.div && self.div.style.display != "none") {
                if (self.__resizing) return;
        ...
        jQuery(window).bind("resize", apply_style);
    }

    if ($('st-widgets-moreoptions')) {
        jQuery("#st-widgets-moreoptions").unbind().bind('click', function() {
            self.toggleOptions();
        });
    }
}
...
Widget.Lightbox.Socialtext.prototype.toggleOptions = function() {
    ...
    if (panel) {
        if (link.innerHTML == loc('More options')) {
            ...
            icon.src = nlw_make_s2_path('/images/st/hide_more.gif');
        }
        else {
            panel.style.display = "none";
            link.innerHTML = loc('More options');
            icon.src = nlw_make_s2_path('/images/st/show_more.gif');
        }
    }
}
