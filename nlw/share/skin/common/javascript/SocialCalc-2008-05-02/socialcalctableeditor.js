...
SocialCalc.ProcessEditorMouseDown = function(e) {
   ...
   if (result.colheader) { // col header - do drag resize
...
SocialCalc.ProcessEditorColsizeMouseDown = function(e, ele, result) {
   ...
   if (document.addEventListener) { // DOM Level 2 -- Firefox, et al
      ...
      ele.setCapture();
      ele.attachEvent("onmousemove", SocialCalc.ProcessEditorColsizeMouseMove);
      ele.attachEvent("onmouseup", SocialCalc.ProcessEditorColsizeMouseUp);
      ele.attachEvent("onlosecapture", SocialCalc.ProcessEditorColsizeMouseUp);
      }
   if (event.stopPropagation) event.stopPropagation(); // DOM Level 2
   else event.cancelBubble = true; // IE 5+
   if (event.preventDefault) event.preventDefault(); // DOM Level 2
...
SocialCalc.ProcessEditorColsizeMouseMove = function(e) {
   ...
   if (newsize < SocialCalc.defaultvalues.minimumcolwidth) newsize = SocialCalc.defaultvalues.minimumcolwidth;
...
SocialCalc.ProcessEditorColsizeMouseUp = function(e) {
   ...
   if (document.removeEventListener) { // DOM Level 2
      ...
      element.detachEvent("onlosecapture", SocialCalc.ProcessEditorColsizeMouseUp);
      element.detachEvent("onmouseup", SocialCalc.ProcessEditorColsizeMouseUp);
      element.detachEvent("onmousemove", SocialCalc.ProcessEditorColsizeMouseMove);
      element.releaseCapture();
      }
   ...
   editor.FitToEditTable();
   editor.context.PrecomputeSheetFonts();
   editor.context.CalculateColWidthData();
   editor.EditorRenderSheet();
   editor.SchedulePositionCalculations();
...
SocialCalc.GridMousePosition = function(editor, clientX, clientY) { 
   ...
   if (editor.headposition) {
      if (clientX < editor.headposition.left) {
         ...
         result.coltoresize = col-(editor.colpositions[col]+editor.colwidth[col]/2>clientX?1:0) || 1; //!!!!! ONLY VISIBLE!!!
         return result;
         }
...
SocialCalc.TCTDragFunctionStart = function(event, draginfo, dobj) {
   ...
   draginfo.thumbstatus.style.fontSize = "small";
